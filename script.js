...
const slowLoadToggle = document.getElementById('slow-load-toggle');
let slowLoadMode = false;
...
// Save theme preference to localStorage and apply
...
function loadSlowLoadPreference() {
    let pref = localStorage.getItem('slowLoadMode');
    if (pref === null) {
        slowLoadMode = true;  // default enabled: Load Together Mode is on by default
        localStorage.setItem('slowLoadMode', 'true');
    } else {
        slowLoadMode = pref === 'true';
    }
    slowLoadToggle.checked = slowLoadMode;
}
loadSlowLoadPreference();

// Save preference to localStorage (do not reload page) -- settings toggle
slowLoadToggle.addEventListener('change', () => {
    const isChecked = slowLoadToggle.checked;
    localStorage.setItem('slowLoadMode', isChecked ? 'true' : 'false');
    // Do NOT reload here; mode will apply on next page load
});

// Settings popup open/close logic with overlay and modal behavior
...
// Prevent closing settings popup on outside click
settingsOverlay.addEventListener('mousedown', (e) => {
    if (e.target === settingsOverlay) {
        e.stopPropagation();
        // Do nothing, do not close popup
    }
});
...
// Refresh collections popup content
collectionsRefresh.addEventListener('click', () => {
    showCollectionsPopup(); // Reload the collections popup content
});
...
// Show the loading spinner and hide the container
function showLoadingSpinner() {
    if (slowLoadMode) {
        loadingOverlay.style.display = 'flex';
        container.style.display = 'none';
        // Add/Sync slow load toggle in loading box
        addSlowLoadToggleToLoadingBox();
    } else {
...
// Add a modern toggle to the loading box for slow load mode
function addSlowLoadToggleToLoadingBox() {
    let loadingBox = document.getElementById('loading-spinner');
    if (!loadingBox) return;
    let existing = document.getElementById('loading-slow-toggle-box');
    if (existing) existing.remove();
    const toggleBox = document.createElement('div');
    toggleBox.id = 'loading-slow-toggle-box';
    toggleBox.style.marginTop = '18px';
    toggleBox.style.display = 'flex';
    toggleBox.style.alignItems = 'center';
    toggleBox.style.justifyContent = 'center';
    toggleBox.innerHTML = `
        <label class="slow-load-label" style="font-size:16px;gap:10px;cursor:pointer;">
            <input type="checkbox" id="loading-slow-toggle" ${slowLoadMode ? 'checked' : ''}>
            <span class="slow-load-slider"></span>
            Load Together Mode
        </label>
    `;
    loadingBox.appendChild(toggleBox);

    // Sync toggle with settings and handle instant reload
    const loadingSlowToggle = document.getElementById('loading-slow-toggle');
    if (loadingSlowToggle) {
        loadingSlowToggle.checked = slowLoadMode;
        loadingSlowToggle.addEventListener('change', () => {
            const isChecked = loadingSlowToggle.checked;
            localStorage.setItem('slowLoadMode', isChecked ? 'true' : 'false');
            // Update settings toggle state without triggering its event
            if (slowLoadToggle) slowLoadToggle.checked = isChecked;
...
// Prevent any clicks outside the loading spinner
loadingOverlay.addEventListener('click', (event) => {
    if (event.target === loadingOverlay) {
        event.stopPropagation(); // Block clicks outside the spinner
    }
});
...
(async function initializeContent() {
    showLoadingSpinner(); // Show spinner before loading starts (conditional)
    try {
        const imdbIds = [
            'tt0232500', 'tt5433140', 'tt0322259', 'tt0463985', 'tt1013752', 'tt1596343', 'tt1905041', 'tt2820852', 'tt4630562', 'tt5433138', 'tt6806448', 'tt18259086', 'tt3566834', 'tt13196080', 'tt0877057', 'tt13706018', 'tt26684398', 'tt4574334', 'tt9389998', 'tt16539454', 'tt10698680', 'tt7838252', 'tt7766378', 'tt29603959', 'tt9150192', 'tt31314296', 'tt21267296', 'tt31434639', 'tt8178634', 'tt12735488', 'tt13927994', 'tt13751694', 'tt15327088', 'tt15654328', 'tt0944947', 'tt5180504', 'tt13443470', 'tt10919420', 'tt28104766', 'tt10048342', 'tt2531336', 'tt13696452', 'tt0903747', 'tt7767422', 'tt16288804', 'tt5753856', 'tt11198330', 'tt23849204', 'tt10189514', 'tt1187043', 'tt5074352', 'tt3417422', 'tt0052572', 'tt0066763', 'tt0109117', 'tt12392504', 'tt0257315', 'tt9432978', 'tt9544034', 'tt12004706', 'tt6077448', 'tt9398466', 'tt0387564', 'tt0432348', 'tt0489270', 'tt0890870', 'tt1132626', 'tt1233227', 'tt1477076', 'tt3348730', 'tt10342730', 'tt21807222', 'tt12412888', 'tt3794354', 'tt0048473', 'tt0048956', 'tt12361178', 'tt15501640', 'tt4430212', 'tt0145487', 'tt0316654', 'tt0413300', 'tt1877830', 'tt0371746', 'tt1228705', 'tt1300854', 'tt0848228', 'tt2395427', 'tt4154756', 'tt4154796', 'tt4154664', 'tt10676048', 'tt10954600', 'tt5095030', 'tt0478970', 'tt0800369', 'tt1981115', 'tt3501632', 'tt10648342'
        ];
        let loadedCount = 0;
        const totalCount = imdbIds.length;
        ...
        if (slowLoadMode) {
            ...
            let buildCollectionsPromise = null;
            for (const imdbId of imdbIds) {
                addContentByImdbId(imdbId).then(() => {
                    loadedCount++;
                    updateLoadingPercent();
                    if (loadedCount === imdbIds.length) {
                        buildCollectionsPromise = buildCollections().then(() => renderCards());
                    }
                });
            }
            // Set initial percent to 0
            updateLoadingPercent();
        }
    } catch (error) {
